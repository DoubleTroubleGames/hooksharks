...
func _physics_process(delta):
	...
	speed2 += speed2.normalized() * acceleration * delta
	...
	if (
		hook != null
		and weakref(hook).get_ref()
		and hook.is_colliding()
...
func die(play_sfx):
	...
	if respawn:
		...
		twn.interpolate_property(
			sprite, "modulate:a", null, 0, 0.4, Tween.TRANS_QUAD, Tween.EASE_IN
		)
		twn.start()
...
func hook_collision(from_hook):
	...
	stunned = true
	pull_dir = (from_hook.player.position - from_hook.position).normalized()
...
func shoot():
	...
	if hook == null and not stunned:
		...
		if not $PowerUps.has_node("MegaHook") and not $PowerUps.has_node("WaterMine"):
			emit_signal("hook_shot", self, hook_dir)  # received in Game.gd
			riders_hook.hide()
		elif $PowerUps.has_node("WaterMine"):
			emit_signal("watermine_released", self)  # received in Game.gd
		elif $PowerUps.has_node("MegaHook"):
			emit_signal("megahook_shot", self, hook_dir)  # received in Game.gd
			riders_hook.texture = original_hook_texture
			riders_hook.scale = ORIGINAL_HOOK_SCALE
			riders_hook.offset = Vector2()
		...
		for action_input in ProjectSettings.get_setting("input/%s" % action_name)["events"]:
			...
			if action_input is InputEventJoypadMotion:
				...
				action_input_pressed = Input.is_joy_button_pressed(
					gamepad_id, action_input.get_button_index()
				)

			elif gamepad_id == -1:  # -1 means this player does not have a gamepad assigned.
				if action_input is InputEventKey:
					action_input_pressed = Input.is_key_pressed(action_input.get_scancode())
...
func _get_movement_direction():
	...
	if gamepad_id != -1:
		direction = Vector2(
			Input.get_joy_axis(gamepad_id, JOY_ANALOG_LX),
	...
	if not dont_collide:
		match area.collision_layer:
			Collision.PLAYER_ABOVE:
				...
				if (
					diving == other_player.diving
					and not invincible
					and not other_player.dont_collide
				):
					die(true)
