...
const N_POINTS = 25
const TEXTURES = {
	"jackie": preload("res://assets/images/characters/jackie/rope_chain.png"),
	...
	"outsider": preload("res://assets/images/characters/outsider/rope_simple.png")
}
...
func straighten(blink = true):
	twn.interpolate_property(self, "max_amplitude", null, 0, .05, Tween.TRANS_BACK, Tween.EASE_IN)
	if blink:
		twn.interpolate_property(
			self, "modulate", Color(1, 1, 1), Color(3, 3, 3), .02, Tween.TRANS_LINEAR, Tween.EASE_IN
		)
		twn.interpolate_property(
			self,
			"modulate",
